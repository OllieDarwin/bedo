...
function Hero({ isMobile }: HeroProps) {
    ...
    return (
        <section className="bg-[#030303] h-screen flex flex-col md:flex-row items-center justify-between overflow-hidden" id="hero">
            <div className="px-6 md:pl-16 w-full md:w-1/2 z-20 mt-24 md:mt-16 text-center md:text-left">
                ...
                <p className="text-base md:text-lg text-zinc-200 mt-2">Harness the power of artifical intelligence to gain an unmatched edge in trading by utilising real-time analysis of news, trends and market movements.</p>
                <button className="btn btn-soft bg-zinc-700 hover:bg-zinc-500 border-0 mt-4">Get Started</button>
            </div>
            <div className="w-4/5 min-h-full z-10 absolute top-0 right-0 pointer-events-none" id="gradient-overlay"></div>
            {/* Hide cards on mobile, show on larger screens */}
            <div className={`flex-row gap-8 ${isMobile ? 'hidden' : 'flex'}`}>
                <div ref={leftColumnRef} className="flex flex-col gap-8 translate-y-32">
                    <img src="cards/1.svg" className="w-40 md:w-64 grayscale hover:grayscale-0 transition-all duration-300" alt="" />
                    <img src="cards/2.svg" className="w-40 md:w-64 grayscale hover:grayscale-0 transition-all duration-300" alt="" />
                    <img src="cards/3.svg" className="w-40 md:w-64 grayscale hover:grayscale-0 transition-all duration-300" alt="" />
                </div>
                <div ref={rightColumnRef} className="flex flex-col gap-8">
                    <img src="cards/4.svg" className="w-40 md:w-64 grayscale hover:grayscale-0 transition-all duration-300" alt="" />
                    <img src="cards/5.svg" className="w-40 md:w-64 grayscale hover:grayscale-0 transition-all duration-300" alt="" />
                    <img src="cards/6.svg" className="w-40 md:w-64 grayscale hover:grayscale-0 transition-all duration-300" alt="" />
                </div>
            </div>
        </section>
    )
